...
fun all_vars :: "p_exp \<Rightarrow> res_id set" where
  "all_vars (ConstExp c) = {}"
| "all_vars (OpExp xop) = {}"
| "all_vars (VarExp v) = (case v of
  VarType x \<Rightarrow> {Var x}
  | LocType x y \<Rightarrow> {Loc x, Loc y}
)"
| "all_vars (PairExp e1 e2) = all_vars e1 \<union> all_vars e2"
| "all_vars (IfExp e1 e2 e3) = all_vars e1 \<union> all_vars e2 \<union> all_vars e3"
| "all_vars (LamExp x e) = all_vars e - {Var x}"
| "all_vars (AppExp e1 e2) = all_vars e1 \<union> all_vars e2"  
...
lemma well_typed_set_restr: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx; all_vars e \<subseteq> ds \<rbrakk> \<Longrightarrow>
  well_typed (set_restr_env env ds) r_s1 e tau r_s2 rx"  
  apply (induct e arbitrary: env r_s1 tau r_s2 rx ds)
   ...
   apply (case_tac "\<not> (all_vars e \<subseteq> ds \<union> {Var x1a})")
    apply (auto)
    (* app case *)
...
lemma well_typed_dom_cover: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx \<rbrakk> \<Longrightarrow> dom_cover env (all_vars e)"    
...
lemma infer_unsat: "\<lbrakk> infer_type env_v ds e tau_v r_sv r_xv r_mv c_list ds'; no_mem_val e;
  dom_ex_cover env_v (all_vars e); unique_env env_v; sub_range env_v ds; \<not> solve_crn_list c_list \<rbrakk> \<Longrightarrow>
  \<not> well_typed env r_s1 e tau r_s2 rx"    
  apply (auto)
    (* first we want to restrict env to only free variables *)
  apply (cut_tac env="env" and e="e" and ds="all_vars e" in well_typed_set_restr)
    apply (auto)
    (* using this we can show that env_v is a specialization of env *)
  apply (cut_tac env_v="env_v" and env="set_restr_env env (all_vars e)" in unique_spec_env)
  ...
  apply (cut_tac env_v="env_v" and env="set_restr_env env (all_vars e)" and t_sub="t_sub" and ds="ds" in restr_spec_env)
    apply (auto)
    (* next we manipulate the permission bounds of e to call the inference lemma *)
  apply (cut_tac env="set_restr_env env (all_vars e)" and ?r_s1.0="r_s1" and e="e" and tau="tau" and ?r_s2.0="r_s2" and rx="rx" in well_typed_end_perm_lbound)
   apply (auto)
  apply (cut_tac env="set_restr_env env (all_vars e)" and r_s="r_s1" and r_ex="r_s1" and r_x="diff_use_env rx r_s1" and e="e"
...
lemma infer_sat: "\<lbrakk> infer_type env_v ds e tau_v r_sv r_xv r_mv c_list ds'; no_mem_val e; solve_crn_list c_list \<rbrakk> \<Longrightarrow>
  (\<exists> env r_s1 tau r_s2 rx. well_typed env r_s1 e tau r_s2 rx)"
  apply (cut_tac c_list="c_list" in solve_crn_list_sat)
    apply (auto)
   apply (rule_tac infer_no_mem_val_crn)
    apply (auto)
  apply (cut_tac tau_n="IntTy" in infer_valid_left)  
     apply (auto)
