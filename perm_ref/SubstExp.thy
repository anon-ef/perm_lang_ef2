...
lemma free_res_vars: "\<lbrakk> Var x \<in> res_vars e \<rbrakk> \<Longrightarrow> x \<in> free_vars e"    
  apply (induct e)
        apply (auto)
  apply (case_tac xa)
   apply (auto)
...
lemma free_res_vars_rev: "\<lbrakk> x \<in> free_vars e \<rbrakk> \<Longrightarrow> Var x \<in> res_vars e"      
  apply (induct e)
        apply (auto)
  apply (case_tac xa)
   apply (auto)
...
lemma subst_res_vars: "\<lbrakk> x \<in> res_vars (subst_exp e y e') \<rbrakk> \<Longrightarrow> (x \<in> res_vars e \<and> x \<noteq> Var y) \<or> x \<in> res_vars e'"    
  apply (induct e)
        apply (auto)
      apply (case_tac xa)
       apply (auto)
...
lemma subst_np_vars: "\<lbrakk> x \<in> non_prim_vars env (subst_exp e y e') \<rbrakk> \<Longrightarrow> (x \<in> non_prim_vars env e \<and> x \<noteq> Var y) \<or> x \<in> non_prim_vars env e'"    
...
lemma subst_type_preserve_no_fv: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx; x \<notin> free_vars e \<rbrakk> \<Longrightarrow> well_typed env r_s1 (subst_exp e x e') tau r_s2 rx"
