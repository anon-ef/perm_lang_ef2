...
| "var_val_crn (LocType x y) tau tau_x = [MemValCrn tau tau_x]"  
...
| "infer_type env ds (VarExp x) tau r_s r_x r_m c_list ds' = (\<exists> a b. env (res_name x) = Some a \<and> tau = VarScheme a \<and> env (owner_name x) = Some b \<and>
  r_s = one_var_env (owner_name x) (XType b) \<and> r_x = r_s \<and> r_m = r_s \<and> c_list = var_val_crn x tau (VarScheme b) \<and> ds' = ds)"
...
lemma well_typed_comp_perms_prec: "\<lbrakk> well_typed env r_s e tau (diff_use_env r_s r_ex) rx \<rbrakk> \<Longrightarrow>
  well_typed env (comp_use_env r_s r_x) e tau (diff_use_env (comp_use_env r_s r_x) r_ex) rx"    
...
lemma well_typed_comp_perms_prec2: "\<lbrakk> well_typed env r_s e tau (diff_use_env r_s r_ex) rx  \<rbrakk> \<Longrightarrow>
  well_typed env (comp_use_env r_x r_s) e tau (diff_use_env (comp_use_env r_x r_s) r_ex) rx"   
...
lemma well_typed_comp_lift_perms: "\<lbrakk> well_typed env (comp_use_env r_sa r_sb) e tau (comp_use_env r_xa r_xb) rx; leq_use_env r_xb r_sb \<rbrakk> \<Longrightarrow>
  well_typed env (comp_use_env r_sa (lift_use_env r_sb r)) e tau (comp_use_env r_xa (lift_use_env r_xb r)) rx"
  apply (induct e arbitrary: env r_sa r_sb tau r_xa r_xb rx)  
    ...
    (* pair case *)
     ...
     apply (auto)
      apply (case_tac "\<not> well_typed env (comp_use_env r_sa r_sb) e1 t1 (comp_use_env r_s2 r_xb) rx1")
       apply (cut_tac r_s="r_s2" and r_x="r_xb" in cancel_comp_use_env2)
        apply (auto)
     apply (rule_tac x="rx2" in exI)
     apply (auto)
        apply (case_tac "\<not> well_typed env (comp_use_env r_s2 r_xb) e2 t2 (comp_use_env r_s3 r_xb) rx2")
    ...
    apply (rule_tac x="comp_use_env r_s2 (lift_use_env r_xb r)" in exI)
    apply (auto)
     apply (case_tac "\<not> well_typed env (comp_use_env r_sa r_sb) e1 BoolTy (comp_use_env r_s2 r_xb) rx'")
      apply (cut_tac r_s="r_s2" and r_x="r_xb" in cancel_comp_use_env2)
       apply (rule_tac r_sb="comp_use_env r_xa r_xb" in trans_leq_use_env)
        apply (rule_tac well_typed_perm_leq)
        apply (auto)
  ...
  apply (rule_tac x="rx1" in exI)
  apply (auto)
   apply (case_tac "\<not> well_typed env (comp_use_env r_sa r_sb) e1 (FunTy t1 tau ra a) (comp_use_env r_s2 r_xb) rx1")
    apply (cut_tac r_s="r_s2" and r_x="r_xb" in cancel_comp_use_env2)
     apply (auto)
  apply (rule_tac x="rx2" in exI)
  apply (rule_tac x="comp_use_env r_s3 (lift_use_env r_xb r)" in exI)
  apply (auto)
   apply (case_tac "\<not> well_typed env (comp_use_env r_s2 r_xb) e2 t1 (comp_use_env r_s3 r_xb) rx2")
    apply (cut_tac r_s="r_s3" and r_x="r_xb" in cancel_comp_use_env2)
     apply (auto)
   apply (cut_tac env="env" and ?r_s1.0="r_s2" and r_c="comp_use_env r_s2 r_xb" and e="e2" in well_typed_incr_start_perm)
     apply (auto)
