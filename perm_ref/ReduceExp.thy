...
fun app_con :: "p_state \<Rightarrow> p_const \<Rightarrow> p_exp \<Rightarrow> p_act \<Rightarrow> p_state \<times> p_exp \<Rightarrow> bool" where
  "app_con s EmptyArrayConst v ax (s', e') = (\<exists> a. ax = MakeAct a \<and> v = ConstExp UnitConst \<and>
    e' = VarExp (LocType a a) \<and> s' = add_env s a (ArrValue []) \<and> fresh_var s a
  )"
| "app_con s NewChanConst v ax (s', e') = (\<exists> c_s c_r. ax = Mk2Act c_s c_r \<and> v = ConstExp UnitConst \<and>
    e' = (PairExp (VarExp (LocType c_s c_s)) (VarExp (LocType c_r c_r))) \<and> s' = add_env (add_env s c_s ChanSValue) c_r (ChanRValue c_s) \<and>
...
fun app_cv :: "p_state \<Rightarrow> p_const \<Rightarrow> p_exp \<Rightarrow> p_exp \<Rightarrow> p_act \<Rightarrow> p_state \<times> p_exp \<Rightarrow> bool" where
  "app_cv s ReadConst e ix ax (s', e') = (\<exists> b a i v arr. e = VarExp (LocType a b) \<and> ix = ConstExp (IConst i) \<and>
    ax = UseAct b \<and> s a = Some (ArrValue arr) \<and>
    read_array arr i = Some v \<and> e' = set_own v b \<and> s' = s)"
| "app_cv s ExtArrayConst e v ax (s', e') = (\<exists> b a arr. e = VarExp (LocType a b) \<and>
    ax = UseAct b \<and> s a = Some (ArrValue arr) \<and>
    e' = e \<and> s' = add_env s a (ArrValue (ext_list arr v)))"
| "app_cv s WriteConst e p ax (s', e') = (\<exists> b a i v arr arr'. e = VarExp (LocType a b) \<and>
    p = PairExp (ConstExp (IConst i)) v \<and> ax = UseAct b \<and>
