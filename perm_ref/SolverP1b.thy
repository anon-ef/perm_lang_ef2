...
lemma reduce_dir_sol_crn2: "\<lbrakk> dir_sol_crn t_subx p_subx c;
  ...
  apply (case_tac c)
       apply (auto)
    (* leq case *)
             apply (rule_tac q="dir_subst_permx t_subx p_subx x31" in trans_leq_perm)
    ...
    (* leq_type case *)
            ...
            apply (case_tac "req_type tau_x")
              apply (auto)
             apply (case_tac "sol_subst_perm p_subx x42")
               apply (auto)
            apply (case_tac "sol_subst_perm p_subx x42")
              apply (auto)
    (* disj crn. first half *)
          ...
          apply (cut_tac px="x52" in eval_dir_leq_permx)
            apply (auto)
          apply (case_tac "dir_subst_permx t_subx p_subx x52")
            apply (auto)
         apply (cut_tac px="x51" in eval_dir_leq_permx)
           apply (auto)
        apply (cut_tac px="x51" in eval_dir_leq_permx)
    ...
    apply (case_tac "eval_permx p_subx (full_subst_permx t_sub p_sub x51)")
      apply (auto)
    (* mini disj crn. *)
   apply (cut_tac px="x61" in eval_dir_leq_permx)
     apply (auto)
   apply (case_tac "dir_subst_permx t_subx p_subx x61")
     apply (auto)
  apply (cut_tac px="x62" in eval_dir_leq_permx)
    apply (auto)
  apply (case_tac "eval_permx p_subx (full_subst_permx t_sub p_sub x62)")
    apply (auto)
  done
...
lemma reduce_subst_crn_corr: "\<lbrakk> sol_sat2 p_subx (reduce_subst_crn t_sub p_sub c);
  ...
  apply (case_tac c)
       apply (auto)
    (* ordering constraint *)
             apply (rule_tac q="eval_permx p_subx (full_subst_permx t_sub p_sub x31)" in trans_leq_perm)
              apply (rule_tac complete_dir_eval_permx)
             apply (simp add: compose_sol_eval_perm)
    (* type order constraint *)
            apply (rule_tac x="full_subst_type (tsub_complete t_sub (psub_compose p_sub p_subx) IntTy) (psub_compose p_sub p_subx) IntTy x41" in exI)
            apply (auto)
    ...
    (* disjointness first half *)
        ...
        apply (cut_tac t_sub="t_sub" and p_sub="p_sub" and p_subx="p_subx" and px="x51" in complete_dir_eval_permx)
        apply (auto)
        apply (case_tac "dir_subst_permx (tsub_complete t_sub (psub_compose p_sub p_subx) IntTy) (psub_compose p_sub p_subx) x51")
          apply (auto)
    ...
    apply (case_tac "dir_subst_permx (tsub_complete t_sub (psub_compose p_sub p_subx) IntTy) (psub_compose p_sub p_subx) x51")
      apply (auto)
    (* semi-disjointness *)
   apply (cut_tac t_sub="t_sub" and p_sub="p_sub" and p_subx="p_subx" and px="x61" in complete_dir_eval_permx)
   apply (auto)
   apply (case_tac "eval_permx p_subx (full_subst_permx t_sub p_sub x61)")
     apply (auto)
  apply (cut_tac t_sub="t_sub" and p_sub="p_sub" and p_subx="p_subx" and px="x62" in complete_dir_eval_permx)
  apply (auto)
  apply (case_tac "dir_subst_permx (tsub_complete t_sub (psub_compose p_sub p_subx) IntTy) (psub_compose p_sub p_subx) x62")
    apply (auto)
  done
