...
| "red_proc_set (s1, ps1) SendAct (s2, ps2) = (\<exists> u1 u2 h1 h2 x_s a_s x_r a_r v.
    ps1 u1 = Some (app_hole h1 (AppExp (AppExp (ConstExp SendConst) (VarExp (LocType x_s a_s))) v)) \<and>
    ps1 u2 = Some (app_hole h2 (AppExp (ConstExp RecvConst) (VarExp (LocType x_r a_r)))) \<and>
