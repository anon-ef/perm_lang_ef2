...
| "well_typed_alt VarRule env r_s1 e tau r_s2 rx = (\<exists> v tau_x. e = VarExp v \<and> env (res_name v) = Some tau \<and>
    env (owner_name v) = Some tau_x \<and> value_req v tau tau_x \<and> leq_use_env (ereq_use_env (owner_name v) tau_x) r_s1 \<and> r_s2 = diff_use_env r_s1 (ereq_use_env (owner_name v) tau_x) \<and>
    rx = diff_use_env (ereq_use_env (owner_name v) tau_x) (ereq_use_env (owner_name v) tau_x))"
...
lemma well_typed_equiv1: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx \<rbrakk> \<Longrightarrow> (\<exists> tag. well_typed_alt tag env r_s1 e tau r_s2 rx)"  
  apply (induct e arbitrary: env r_s1 r_s2 tau rx)
    ...
    (* var case *)
      ...
      apply (auto)
      apply (rule_tac x="diff_use_env (diff_use_env r_s1 (ereq_use_env (owner_name x) tau_x)) r_ex" in exI)
      apply (auto)
       apply (rule_tac x="diff_use_env (diff_use_env (ereq_use_env (owner_name x) tau_x) (ereq_use_env (owner_name x) tau_x)) r_ex" in exI)
       apply (auto)
